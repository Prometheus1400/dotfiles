...
knsget() {
    kubectl config get-contexts | awk 'FNR == 2 {print}' | awk '{print $5}' 
}
klog() {
    pods=$(kubectl get pods --all-namespaces -o custom-columns=NAME:.metadata.name,NAMESPACE:.metadata.namespace --no-headers)
    if [ -z "$pods" ]; then
        echo "No pods found in the cluster."
        return
    fi
    selected_pod=$(echo "$pods" | fzf --header="Select a pod to tail logs")
    if [ -z "$selected_pod" ]; then
        echo "No pod selected."
        return
    fi
    namespace=$(echo $selected_pod | awk '{print $2}')
    pod_name=$(echo $selected_pod | awk '{print $1}')
